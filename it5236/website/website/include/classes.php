...
class Application {
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://bff7wiix8a.execute-api.us-east-1.amazonaws.com/registor/registration";
			$data = array(
				'userid'=>$userid,
				'username'=>$username,
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key:kdriwghg2jalMyCQOFxuXa5rdtNlCapa7hL1Chu8'));
	...
	$otp= bin2hex(random_bytes(3));
		...
		$headers ='From: webmaster@annesar.me' . "\r\n" .
		'Reply-To: webmaster@annesar.me' . "\r\n";
